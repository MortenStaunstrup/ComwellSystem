...
public class UsersController : ControllerBase
{
    ...
    public async Task<IActionResult>
        Login(string email, string password) // (bruger iactionresult, så den kan returne forskelligt)
    {
        var user = await _repo.Login(email, password); // Tjekker om brugeren findes
        if (user == null)
        {
            return Unauthorized(
                "Forkert email eller adgangskode... prøv igen "); // Hvis ikke, send et eller andet svar.
        }
